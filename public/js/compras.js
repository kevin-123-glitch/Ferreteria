$(document).ready(function () {
    ...
    /* document.getElementById('selDocumentoVenta').addEventListener('change', function () {
         ...
         if (tipoDocumentoSeleccionado) {
             ...
             fetch(`http://localhost:3060/api/compra/serie/${tipoDocumentoSeleccionado}`)
    ...
    function fetchSuggestions(termino, autoSelect = false) {
        fetch(`${apiBaseUrl}/compra/buscarProducto?q=${encodeURIComponent(termino)}`)
    ...
    $('#proveedor-input').on('input', function () {
        var termino = $(this).val();
        if (termino.length > 1) {
            fetchSuggestionsProveedor(termino); // Llamar a la función para obtener sugerencias
        } else {
            limpiarSugerenciasProveedor(); // Limpiar sugerencias si no hay suficiente texto
        }
    });

    // Evento para manejar el Enter como una búsqueda
    $('#proveedor-input').on('keypress', function (e) {
        if (e.which === 13) { // Si el usuario presiona Enter
            e.preventDefault();
            var termino = $(this).val();
            if (termino.length > 1) {
                fetchSuggestionsProveedor(termino, true); // Forzar la selección si solo hay un resultado
            }
        }
    });

    // Función para hacer la solicitud de búsqueda al servidor
    function fetchSuggestionsProveedor(termino, autoSelect = false) {
        fetch(`${apiBaseUrl}/compra/buscarProveedor?q=${encodeURIComponent(termino)}`)

            .then(response => response.json()) // Convertir la respuesta en JSON
            .then(data => {
                mostrarSugerenciasProveedor(data, autoSelect); // Mostrar sugerencias en la lista
            })
            .catch(error => console.error('Error en la solicitud de búsqueda:', error)); // Manejo de errores
    }

    function mostrarSugerenciasProveedor(proveedores, autoSelect) {
        var sugerencias = $('#suggestionsproveedor');
        sugerencias.empty(); // Vaciar la lista de sugerencias anteriores

        if (proveedores.length > 0) {
            proveedores.forEach(proveedor => {
                var item = $('<li class="list-group-item"></li>')
                    .text(`${proveedor.Nombre_empresa} - ${proveedor.Numero_documento}`)
                    .data('proveedor', proveedor); // Asociar el objeto proveedor al ítem

                item.on('click', () => seleccionarProveedor(proveedor)); // Seleccionar proveedor al hacer clic
                sugerencias.append(item); // Añadir el item a la lista de sugerencias
            });
            sugerencias.show(); // Mostrar la lista de sugerencias

            if (autoSelect && proveedores.length === 1) {
                seleccionarProveedor(proveedores[0]); // Seleccionar automáticamente si hay un único resultado
            }
        } else {
    ...
    function limpiarSugerenciasProveedor() {
        $('#suggestionsproveedor').empty().hide(); // Vaciar y ocultar la lista de sugerencias
    }

    // Función para manejar la selección de un proveedor
    function seleccionarProveedor(proveedor) {
        $('#proveedor-input').val(`${proveedor.Nombre_empresa} - ${proveedor.Numero_documento}`); // Poner el nombre del proveedor en el campo de texto*/
        $('#Id_obtenido_proveedor').val(`${proveedor.id_proveedor}`); // Poner el id_proveedor en el campo de texto

        limpiarSugerenciasProveedor(); // Limpiar sugerencias una vez seleccionado
    }
    ...
    function seleccionarProducto(producto) {
        ...
        $('#product-price').val(producto.Precio_C);
        ...
        if (existingRow.length > 0) {
            ...
            actualizarTotal(); // Asegúrate de actualizar el total después de agregar el producto
        }
    }
    ...
    function agregarProducto(producto) {
        ...
        var importe = producto.Precio_C * cantidad;
