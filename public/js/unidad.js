    ...
    fetch(`${apiBaseUrl}/unidad`)
        ...
        .then(data => {
            ...
            if (Array.isArray(data) && data.length > 0) {
                data.sort((a, b) => a.Id_unidad - b.Id_unidad);

                data.forEach((unidades_medida, index) => {
                    const row = document.createElement('tr');
                    row.setAttribute('data-id', unidades_medida.Id_unidad);
                    ...
                    row.innerHTML = `
                        <td>${unidades_medida.Id_unidad || '--'}</td>
                        <td>${unidades_medida.Nombre || '--'}</td>
                        <td>${unidades_medida.Descripcion || '--'}</td>
                        <td>
                            <button class="btn btn-warning btn-sm edit-btn" onclick="abrirModalEditar(${unidades_medida.Id_unidad})">
                                <i class="fas fa-pencil-alt"></i>
                            </button>
                            <button class="btn btn-danger btn-sm delete-btn" onclick="eliminar(${unidades_medida.Id_unidad})">
                ...
                tableElement.DataTable({
                    ...
                    columnDefs: [
                        {
                            targets: [], // Índices de las columnas a ocultar en vistas móviles
...
function generatePDF() {
    ...
    const props = {
        ...
        business: {
            name: "Reporte de unidades de medidas",
        ...
        invoice: {
            ...
            header: [
                { title: "ID", style: { width: 15 } },
                { title: "Unidad de medida", style: { width: 50 } },
                { title: "Descripcion", style: { width: 125 } },
...
function generateExcel() {
    document.getElementById('buttons-excel').addEventListener('click', function () {
        ...
        window.location.href = `${apiBaseUrl}/unidad/exportExcel`;
            ...
            fetch(`${apiBaseUrl}/unidad/${id}`, {
                ...
                .then(message => {
                    Swal.fire({
                        ...
                        obtenerUnidad(); // Actualiza la lista después de eliminar
                    });
                })
                .catch(error => {
...
function abrirModalEditar(id) {
    ...
    document.getElementById('modalLabel').textContent = 'Modificar datos de la unidad de medida';

    const row = document.querySelector(`#diseñoTable tr[data-id="${id}"]`);

    if (row) {
        //Obtener los valores de la fila y validarlos
        const id_unidad = validateField(row.querySelector('td:nth-child(1)').textContent.trim());
        ...
        var modalElement = document.getElementById('registrarUnidadModal');
...
function cargarSiguienteId() {
    fetch(`${apiBaseUrl}/unidad/obtener`)
        .then(response => response.json())
        .then(data => {
            document.getElementById('id_unidad').value = data.siguienteId;
        })
...
async function Registrar(event) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                const response = await fetch(`${apiBaseUrl}/unidad/registrar`, {
                    method: 'POST',
                    headers: {
                ...
                if (response.ok) {
                    ...
                    const modal = bootstrap.Modal.getInstance(document.getElementById('registrarUnidadModal'));
                    modal.hide();
                ...
                Swal.fire('Error', 'Error al actualizar la undiad de medida', 'error');
            }
        }
    });
}
...
async function Modificar(event) {
    ...
    try {
        const response = await fetch(`${apiBaseUrl}/unidad/modificar/${Id_unidad}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        console.error('Error al enviar la solicitud:', error);
        Swal.fire('Error', 'Error al actualizar la unidad', 'error');
    }
}
