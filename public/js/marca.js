    ...
    fetch(`${apiBaseUrl}/marca`)
        ...
        .then(data => {
            ...
            if (Array.isArray(data) && data.length > 0) {
                data.sort((a, b) => a.Id_marca - b.Id_marca);
                ...
                data.forEach((marca, index) => {
                    ...
                    row.innerHTML = `
                        ...
                        <td>
                            <button class="btn btn-warning btn-sm edit-btn" onclick="abrirModalEditar(${marca.Id_marca})">
                                <i class="fas fa-pencil-alt"></i>
                            </button>
                            <button class="btn btn-danger btn-sm delete-btn" onclick="eliminar(${marca.Id_marca})">
...
function generateExcel() {
    document.getElementById('buttons-excel').addEventListener('click', function () {
        ...
        window.location.href = `${apiBaseUrl}/marca/exportExcel`;
            ...
            fetch(`${apiBaseUrl}/marca/${id}`, {
                ...
                .then(message => {
                    Swal.fire({
                        ...
                        obtenerMarca(); // Actualiza la lista después de eliminar
                    });
                })
                .catch(error => {
...
function abrirModalEditar(id) {
    ...
    if (row) {
        ...
        const id_marca = validateField(row.querySelector('td:nth-child(1)').textContent.trim());
        ...
        var modalElement = document.getElementById('registrarMarcaModal');
...
function cargarSiguienteId() {
    fetch(`${apiBaseUrl}/marca/obtener`)
        .then(response => response.json())
        .then(data => {
            document.getElementById('id_marca').value = data.siguienteId;
        })
...
async function Registrar(event) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                const response = await fetch(`${apiBaseUrl}/marca/registrar`, {
                    method: 'POST',
                    headers: {
                ...
                if (response.ok) {
                    const modal = bootstrap.Modal.getInstance(document.getElementById('registrarMarcaModal'));
                    modal.hide();
                    ...
                    Swal.fire({
                        ...
                        text: 'La marca se guardó exitosamente.',
                        icon: 'success',
                        timer: 1000,               // Tiempo en milisegundos (3000 ms = 3 segundos)
                        showConfirmButton: false    // Ocultar botón de confirmación
                    }).then(() => {
...
async function Modificar(event) {
    ...
    try {
        const response = await fetch(`${apiBaseUrl}/marca/modificar/${Id_marca}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
