...
const checkNumeroDocumento = (numero_documento) => {
    return new Promise((resolve, reject) => {
        ...
        const sql = "SELECT COUNT(*) AS count FROM proveedor WHERE Numero_documento = ? AND Numero_documento IS NOT NULL";
...
const checkCorreo = (correo) => {
    return new Promise((resolve, reject) => {
        ...
        const sql = "SELECT COUNT(*) AS count FROM proveedor WHERE Correo = ? AND Correo IS NOT NULL";
...
const checkTelefono = (telefono) => {
    return new Promise((resolve, reject) => {
        ...
        const sql = "SELECT COUNT(*) AS count FROM proveedor WHERE Telefono = ? AND Telefono IS NOT NULL";
...
exports.registrarProveedor = async (req, res) => {
    const {
        Id_tdoc_proveedor, numero_documento, nombre, contacto, telefono, direccion, provincia, distrito, correo, rubro
    } = req.body;

    // Verificar si los campos obligatorios están presentes
    let errors = {};
    if (!Id_tdoc_proveedor) errors.Id_tdoc_proveedor = 'Tipo de documento es requerido';
    ...
    try {
        ...
        const insertSql = `INSERT INTO proveedor (ID_Tipo_Doc, Numero_documento, Nombre_empresa, Contacto, Telefono, Direccion, Provincia, Distrito, Correo, Rubro) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
        conexion.query(insertSql, [
            ...
            if (err) {
                ...
                return res.status(500).json({ error: 'Error al registrar el proveedor', details: err.message });
            }
...
exports.modificarProveedor = (req, res) => {
    ...
    conexion.query(sql, [
        ...
        if (err) {
            ...
            res.status(500).json({ error: 'Error al modificar el proveedor', details: err.message });
        } else {
            ...
            if (result.affectedRows === 0) {
                ...
                res.status(200).json({ message: 'Proveedor modificado correctamente', affectedRows: result.affectedRows });
            }
        }
    });
};
...
exports.exportExcel = async (req, res) => {
    ...
    conexion.query(query, async (error, results) => {
        ...
        const worksheet = workbook.addWorksheet('proveedor');
        ...
        const headers = [
            ...
            { header: 'Empresa', key: 'Nombre_empresa', width: 15 },
            ...
            { header: 'Rubro', key: 'Rubro', width: 30 },
        ...
        res.setHeader('Content-Disposition', 'attachment; filename=proveedor.xlsx');
