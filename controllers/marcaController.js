...
exports.obtenerSiguienteId = (req, res) => {
    const query = 'SELECT MAX(Id_marca) AS ultimo_id FROM marca';
...
const checkNombre = (nombre) => {
    return new Promise((resolve, reject) => {
        ...
        const sql = "SELECT COUNT(*) AS count FROM marca WHERE Nombre = ? AND Nombre IS NOT NULL";
...
exports.registrarMarca = async (req, res) => {
    ...
    let errors = {};
    if (!nombre) errors.nombre = 'El Nombre de la marca es requerida';
    ...
    try {
        ...
        if (docExists) responseErrors.nombre = 'El nombre de la marca ya está registrado';
        ...
        const insertSql = `INSERT INTO marca (Id_marca, nombre, descripcion) VALUES (?, ?, ?)`;
        conexion.query(insertSql, [
            id_marca, nombre, descripcion
        ], (err, results) => {
            if (err) {
                console.error('Error al insertar el registro:', err);
                return res.status(500).json({ error: 'Error al registrar la marca', details: err.message });
            }
...
exports.modificarMarca = (req, res) => {
    ...
    let sql = `UPDATE marca SET 
        ...
        Nombre = ?, 
        Descripcion = ?
        WHERE Id_marca = ?`; // Usamos `Numero_documento` para buscar el registro a actualizar

    conexion.query(sql, [
        data.Id_marca,
        data.Nombre,
        data.Descripcion,
        id_marca // El número de documento del registro que quieres modificar

    ], function (err, result) {
        if (err) {
            console.error('Error en la consulta:', err);
            res.status(500).json({ error: 'Error al modificar la marca', details: err.message });
        } else {
...
exports.exportExcel = async (req, res) => {
    ...
    conexion.query(query, async (error, results) => {
        ...
        const headers = [
            { header: 'ID', key: 'Id_marca', width: 10 },
            { header: 'Marca', key: 'Nombre', width: 50 },
        ...
        res.setHeader('Content-Disposition', 'attachment; filename=marca.xlsx');
